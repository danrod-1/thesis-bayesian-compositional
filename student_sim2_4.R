...
evaluate_model_betas <- function(fit, beta_matrix) {
  ...
  recovered_names <- intersect(active_names, summary_df$Parameter[summary_df$selected])
  n_sign_matches <- sum(sign(true_param_values[recovered_names]) == 
                          sign(summary_df$Mean[match(recovered_names, summary_df$Parameter)]), na.rm = TRUE)
...
generate_beta <- function(
  ...
  C_eff <- C - 1  # exclude reference component
  beta <- matrix(0, nrow = C_eff, ncol = P)
  
  for (k in 1:C_eff) {
    informative_vars <- shared_covariates
    
    remaining_pool <- setdiff(2:P, informative_vars)  # exclude intercept
...
generate_student <- function(
  ...
  C_eff <- C - 1
  Sigma <- (1 - rho) * diag(C_eff) + rho * matrix(1, C_eff, C_eff)
  
  Z_latent <- matrix(0, nrow = n_obs, ncol = C_eff)
  
  for (i in 1:n_obs) {
    epsilon_i <- rmvt(1, sigma = Sigma, df = nu)
    Z_latent[i, ] <- X[i, , drop = FALSE] %*% t(beta[1:C_eff, ]) + epsilon_i
  }
...
fit_normal <- function(
  ...
  sigma_summary <- summarise_draws(
    sigma_draws,
    "mean", "sd", ~quantile(.x, probs = c(0.025, 0.975))
  ) |>
    dplyr::mutate(model = model_name, sim = sim_id, true_value = as.vector(Sigma_true))
  
  Sigma_mean <- matrix(sigma_summary$mean, nrow = D, byrow = FALSE)
  frobenius_error <- sqrt(sum((Sigma_mean - Sigma_true)^2))
  
  lock_pred <- lock(file.path(output_dir, "prediction_results.lock"))
  readr::write_csv(
    pred_df,
    file.path(output_dir, "prediction_results.csv"),
    append = file.exists(file.path(output_dir, "prediction_results.csv"))
  ...
  lock_beta <- lock(file.path(output_dir, "beta_results.lock"))
  readr::write_csv(
    beta_selection_eval,
    file.path(output_dir, "beta_results.csv"),
    append = file.exists(file.path(output_dir, "beta_results.csv"))
  ...
  lock_sigma <- lock(file.path(output_dir, "sigma_results.lock"))
  readr::write_csv(
    tibble::tibble(sim = sim_id, frobenius_error = frobenius_error, model = model_name),
    file.path(output_dir, "sigma_results.csv"),
    append = file.exists(file.path(output_dir, "sigma_results.csv"))
  )
  unlock(lock_sigma)
...
fit_student <- function(
  ...
  nu_summary <- summarise_draws(
    as_draws_df(fit$draws("nu")),
    "mean", "median", "sd", ~quantile(.x, probs = c(0.025, 0.975))
  ) |>
    dplyr::mutate(model = model_name, sim = sim_id, true_value = nu_true)
  ...
  lock_nu <- lock(file.path(output_dir, "nu_results.lock"))
  readr::write_csv(
    nu_summary,
    file.path(output_dir, "nu_results.csv"),
    append = file.exists(file.path(output_dir, "nu_results.csv"))
  )
  unlock(lock_nu)
  ...
  rm(fit, mu_train, mu_test,
     pred_train, pred_test, pred_df,
     beta_selection_eval, nu_summary,
     sigma_summary, sigma_draws)
  gc()
}

run_simulations_alr_student <- function(
    start_sim_id,
    end_sim_id,
    C = 3,
    P = 50,
    ...
    seed = 1,
    beta_seed = 10,
    output_dir = "results_alr_student/",
    iter_sampling = 1000,
    iter_warmup = 500,
    chains = 4,
    parallel_chains = 4,
  ...
  for (sim_id in start_sim_id:end_sim_id) {
    ...
    data <- generate_student(
      ...
      beta = beta, nu = nu, rho = rho, seed = sim_seed
    )
